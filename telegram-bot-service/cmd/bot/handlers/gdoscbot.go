...
const _gdOSCommercialBotName = "GDOSCommercialBot"

type GDOSCommercialBot struct {
	TelegramBot *telebot.Bot
	userClient  *client.UserServiceClient
}

func NewGDOSCommercialBot(configs *config.EnvVars, userClient *client.UserServiceClient) (*GDOSCommercialBot, error) {
	pref := telebot.Settings{
		Token:  configs.GDOSCommercialToken,
	...
	return &GDOSCommercialBot{
		TelegramBot: bot,
		userClient:  userClient,
	}, nil
}

func (g *GDOSCommercialBot) UploadImage(c telebot.Context) error {
	...
	err = writer.WriteField("bot_name", _gdOSCommercialBotName)
	if err != nil {
		log.Println("err: ", err)
		return c.Send(_errIdentifyingUser)
	}
